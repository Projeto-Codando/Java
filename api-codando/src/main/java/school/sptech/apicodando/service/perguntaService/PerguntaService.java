    ...
    public List<Integer> inserirDadosIniciaisSeNecessario(List<Integer> idsAulas) {
        if (idsAulas != null) {
            List<Pergunta> perguntas = List.of(
                    ...
                    new Pergunta("Durante uma celebração na floresta, os macacos querem decidir que tipo de dança fazer. Eles têm uma variável tipoDanca onde a representa a dança do fogo, b representa a dança da chuva, e c representa a dança do sol. Qual switch case eles usariam?", aulaService.buscarPorId(idsAulas.get(1))),
                    new Pergunta("Os macacos estão escolhendo frutas para um banquete. Eles têm uma variável fruta onde 1 representa bananas, 2 representa maçãs, e 3 representa mangas. Qual switch case eles usariam para imprimir a fruta escolhida? ", aulaService.buscarPorId(idsAulas.get(1))),
                    new Pergunta("Os macacos estão decidindo a cor das flores para decorar suas casas. Eles têm uma variável corFlor onde red representa flores vermelhas, blue representa flores azuis, e yellow representa flores amarelas. Como usariam um switch case para decidir a cor? ", aulaService.buscarPorId(idsAulas.get(1))),
                    new Pergunta("Em um jogo de esportes na selva, os macacos precisam decidir qual jogo jogar baseado no valor da variável jogo, onde 1 representa futebol, 2 representa basquete, e 3 representa vôlei. Qual switch case eles usariam? ", aulaService.buscarPorId(idsAulas.get(1))),

                    new Pergunta("Um macaco encontrou um cacho de bananas com 15 bananas e deu 3 bananas para seu amigo. Qual é o número de bananas restantes?", aulaService.buscarPorId(idsAulas.get(2))),
                    new Pergunta("Os macacos coletaram 8 cocos de uma árvore e encontraram mais 5 no chão. Quantos cocos eles têm ao todo?", aulaService.buscarPorId(idsAulas.get(2))),
                    new Pergunta("Em um experimento, os macacos estão medindo a altura de uma árvore que cresce 2 metros a cada ano. Se a árvore tem 5 metros agora, qual será a altura da árvore em 3 anos?", aulaService.buscarPorId(idsAulas.get(2))),
                    new Pergunta("Um macaco está carregando 12 maçãs e quer dividi-las igualmente entre 4 amigos. Quantas maçãs cada amigo receberá?", aulaService.buscarPorId(idsAulas.get(2))),
                    new Pergunta("Os macacos precisam multiplicar a quantidade de sementes de cada fruta que coletaram. Eles têm 5 frutas, e cada fruta tem 4 sementes. Qual é o total de sementes?", aulaService.buscarPorId(idsAulas.get(2)))
            );
